...
- **Handing over to the dashboard** — `Go to Dashboard` runs as separate beats rather than one cut: the CTA works for a moment under a spinner, then the dialog leaves as the whole page rises in behind it — top nav, sidebar and content as one piece — and the Setup guide follows a beat later. The guide is held out of the scrim for the whole dialog, so it never reads through it. The arrival rides `.surface`, not `.content`: the nav sits outside `.content`, and `.content` already owns `route-in`, so overriding its animation would re-fire that animation on the way back out.
